...
def compute_peer_weights_gbm(model, X_train, X_test, y_train, learning_rate):
    ...
    diff_pred = k - y_hat
    assert np.allclose(diff_pred, np.zeros((1, n_train)), atol=1e-4), "Mismatch in GBM predictions"
    print("   ---> GBM predictions validated.")

    # Validate W predictions
    y_test_hat = model.predict(X_test)
    diff_pred = k_test - y_test_hat
    assert np.allclose(diff_pred, np.zeros((1, S_test)), atol=1e-4), "Mismatch in out-of-sample GBM predictions!"
...
for month in tqdm(unique_months, desc="Processing months", position=0, leave=True):
    ...
    for fold in tqdm(range(1, 6), desc="Processing folds", position=1, leave=True):
        ...
        fold_outcomes_path = f"/Volumes/HugoHardDrive/05. Results/GBM/v2s/Outcomes/outcomes_{month.strftime('%Y-%m')}_fold{fold}.csv"
        ...
        # Save the W matrix
        W_path = f"/Volumes/HugoHardDrive/05. Results/GBM/v2s/Peer weights/peer_weights_{month.strftime('%Y-%m')}_fold{fold}.csv"
...
# Save the results
results.to_csv("/Volumes/HugoHardDrive/05. Results/GBM/v2s/results.csv", index=False)
