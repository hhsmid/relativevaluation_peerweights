...
# Save an unscaled copy of book_equity before scaling
data = pd.concat([
    data,
    data['book_equity'].rename('book_equity_unscaled'),
], axis=1)
...
def optimize_clusters_gmm(train_set, val_set, predictors, max_clusters=None, iterations=5, n_jobs=cores):
    """
    ...
    - max_clusters (int, optional): Maximum number of clusters to test. Defaults to None.
    - iterations (int, optional): Number of clustering trials. Defaults to 5.
    ...
    def evaluate_gmm(iteration):
        """Fit GMM, compute peer weights, and calculate RMSE."""
        try:
            ...
            peer_weights = compute_peer_weights_gmm(train_set, val_set, gmm, predictors)
            if peer_weights is None:
                return iteration, None, k, np.inf  

            # Compute out-of-sample predictions
            y_val_hat = compute_oos_predictions(train_set, val_set, peer_weights, ['ln_m2b'])
            m2b_pred = np.exp(y_val_hat)

            # Calculate predicted market cap
            book_equity_val = val_set['book_equity_unscaled']
            mktcap_pred = np.maximum(m2b_pred * book_equity_val.values, 0)

            # Calculate RMSE
            val_rmse = np.sqrt(mean_squared_error(y_val_actual, mktcap_pred))
            return iteration, gmm, k, val_rmse
    ...
    results = Parallel(n_jobs=n_jobs)(delayed(evaluate_gmm)(i) for i in range(iterations))

    # Sort results by iteration order to ensure proper output
    results.sort(key=lambda x: x[0])

    # Print RMSEs over iterations
    for i, _, k, rmse in results:
        if k is not None:
            print(f"Iteration {i+1}: {k} clusters → Validation RMSE: {rmse:.5f}")
...
# Function to compute peer weights
def compute_peer_weights_gmm(train_set, test_set, clustering_model, predictors):
    """
    Computes peer weights based on clustering probabilities from a GMM model.

    Parameters:
    - train_set (pd.DataFrame): Training dataset containing firm-level data.
    - test_set (pd.DataFrame): Test dataset containing firm-level data.
    - clustering_model (GaussianMixture): Pre-trained GMM clustering model.
    - predictors (list of str): Variables used for clustering.

    Returns:
    - pd.DataFrame: Peer weight matrix (rows: training firms, columns: test firms).
    """
    X_train = train_set[predictors].values
    X_test = test_set[predictors].values

    probs_train = clustering_model.predict_proba(X_train)
    probs_test = clustering_model.predict_proba(X_test)

    n_train = X_train.shape[0]
    n_test = X_test.shape[0]
    peer_weights = np.zeros((n_train, n_test))

    for s in range(n_test):
        joint_probs = (probs_train * probs_test[s, :]).sum(axis=1)
        norm = joint_probs.sum()
        if norm > 0:
            peer_weights[:, s] = joint_probs / norm
        else:
            peer_weights[:, s] = 1.0 / n_train

    return pd.DataFrame(peer_weights, index=train_set['gvkey'], columns=test_set['gvkey'])


# Function to compute OOS predictions
def compute_oos_predictions(train_set, test_set, peer_weights, target_variable):
    """
    Computes out-of-sample predictions using peer weights.
    
    Parameters:
    - train_set (pd.DataFrame): Training dataset with target variable.
    - test_set (pd.DataFrame): Test dataset.
    - peer_weights (pd.DataFrame): Peer weight matrix (output of `compute_peer_weights_gmm`).
    - target_variable (str): Name of the target variable to predict.
    
    Returns:
    - pd.Series: Predicted values for the test set indexed by 'gvkey'.
    """
...
for month in tqdm(unique_months, desc="Processing months", position=0, leave=True):
    ...
    for fold in tqdm(range(1, 6), desc="Processing folds", position=1, leave=True):
        ...
        # Optimize clusters for the current fold using GMM
        clustering_model, best_n_clusters, _ = optimize_clusters_gmm(train_set, val_set, predictors)
        ...
        peer_weights = compute_peer_weights_gmm(train_set, test_set, clustering_model, predictors)
        if peer_weights is None:
        ...
        W_path = f"/Users/hhsmid/Desktop/Hugo EUR/0. Master Thesis/05. Results/GMM/m2b/Peer weights/peer_weights_{month.strftime('%Y-%m')}_fold{fold}.csv"
        ...
        # Calculate predicted market cap
        book_equity_test = test_set['book_equity_unscaled']
        ...
        fold_outcomes_path = f"/Users/hhsmid/Desktop/Hugo EUR/0. Master Thesis/05. Results/GMM/m2b/Outcomes/outcomes_{month.strftime('%Y-%m')}_fold{fold}.csv"
...
# Save the results
results.to_csv("/Users/hhsmid/Desktop/Hugo EUR/0. Master Thesis/05. Results/GMM/m2b/results.csv", index=False)
