...
for month in tqdm(unique_months, desc="Processing months", position=0, leave=True):
    ...
    for fold in tqdm(range(1, 6), desc="Processing folds", position=1, leave=True):
        ...
        W_path = f"/Results/GMM/m2b/Peer weights/peer_weights_{month.strftime('%Y-%m')}_fold{fold}.csv"
        ...
        fold_outcomes_path = f"/Results/GMM/m2b/Outcomes/outcomes_{month.strftime('%Y-%m')}_fold{fold}.csv"
...
# Save the results
results.to_csv("/Results/GMM/m2b/results.csv", index=False)
