...
for month in tqdm(unique_months, desc="Processing months", position=0, leave=True):
    ...
    for fold in tqdm(range(1, 6), desc="Processing folds", position=1, leave=True):
        ...
        W_path = f"/Users/hhsmid/Desktop/Hugo EUR/0. Master Thesis/05. Results/FCM/v2s/Peer weights/peer_weights_{month.strftime('%Y-%m')}_fold{fold}.csv"
        ...
        fold_outcomes_path = f"/Users/hhsmid/Desktop/Hugo EUR/0. Master Thesis/05. Results/FCM/v2s/Outcomes/outcomes_{month.strftime('%Y-%m')}_fold{fold}.csv"
...
# Save the results
results.to_csv("/Users/hhsmid/Desktop/Hugo EUR/0. Master Thesis/05. Results/FCM/v2s/results.csv", index=False)
