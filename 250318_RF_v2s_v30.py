...
data = data[
    (data['month'] >= pd.to_datetime('1997-09-01')) & 
    (data['month'] < pd.to_datetime('2024-01-01'))
]
...
def optimize_model_rf(X_train, y_train, X_val, y_val, n_estimators_max=n_estimators_max, early_stopping_rounds=early_stopping_rounds):
    ...
    for n_estimators in range(1, n_estimators_max + 1):
        ...
        if no_improvement_count >= early_stopping_rounds:
            print(f"Early stopping at n_estimators={n_estimators} (Best RMSE={best_val_rmse:.4f})", flush=True)
            break
...
def compute_peer_weights_rf(model, X_train, X_test, y_train):
    ...
    for m, tree in enumerate(model.estimators_):
        ...
        zero_denom_count = 0  # Count test samples with zero denominator for this tree
            ...
            if denom > 0:
                ...
                if zero_denom_count <= 5:
                    print(f"DEBUG: Tree {m}, Test Sample {s}: leaf_id={leaf_id}, "
                          ...
                          f"denom={denom}")
                W[:, s] += 0.0  # This line is not necessary, but added for clarity.
...
for month in tqdm(unique_months, desc="Processing months", position=0, leave=True):
    ...
    for fold in tqdm(range(1, 6), desc="Processing folds", position=1, leave=True):
        ...
        fold_outcomes_path = f"/Volumes/HugoHardDrive/05. Results/RF/v2s/Outcomes/outcomes_{month.strftime('%Y-%m')}_fold{fold}.csv"
        ...
        # Save the W matrix
        W_path = f"/Volumes/HugoHardDrive/05. Results/RF/v2s/Peer weights/peer_weights_{month.strftime('%Y-%m')}_fold{fold}.csv"
...
# Save the results
results.to_csv("/Volumes/HugoHardDrive/05. Results/RF/v2s/results.csv", index=False)
