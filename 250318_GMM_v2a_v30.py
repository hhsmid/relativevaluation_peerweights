...
def optimize_clusters_gmm(train_set, val_set, predictors, max_clusters=None, iterations=5, n_jobs=cores):
    ...
    def evaluate_gmm(iteration):
        ...
        try:
            ...
            y_val_hat = compute_oos_predictions(train_set, val_set, peer_weights, ['ln_v2a'])
...
for month in tqdm(unique_months, desc="Processing months", position=0, leave=True):
    ...
    for fold in tqdm(range(1, 6), desc="Processing folds", position=1, leave=True):
        ...
        # Optimize clusters for the current fold using GMM with Ward linkage
        ...
        W_path = f"/Users/hhsmid/Desktop/Hugo EUR/0. Master Thesis/05. Results/GMM/v2a/Peer weights/peer_weights_{month.strftime('%Y-%m')}_fold{fold}.csv"
        ...
        fold_outcomes_path = f"/Users/hhsmid/Desktop/Hugo EUR/0. Master Thesis/05. Results/GMM/v2a/Outcomes/outcomes_{month.strftime('%Y-%m')}_fold{fold}.csv"
...
# Save the results
results.to_csv("/Users/hhsmid/Desktop/Hugo EUR/0. Master Thesis/05. Results/GMM/v2a/results.csv", index=False)
