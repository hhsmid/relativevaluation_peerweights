...
# %% Utility functions
# Function to determine the optimal number of clusters for K-Means clustering
def optimize_clusters_kmeans(train_set, val_set, predictors, max_clusters=None, iterations=5, n_jobs=cores):
    """
    Determines the optimal number of clusters for K-Means clustering using validation RMSE.
    ...
    - target_variable (list or str): The target variable name(s) (e.g. ['ln_m2b']).
    ...
    def evaluate_kmeans(iteration):
        try:
            ...
            # Compute peer weights on the validation set using the current clustering model
            peer_weights = compute_peer_weights_kmeans(train_set, val_set, kmeans, predictors)
            if peer_weights is None:
                return iteration, None, k, np.inf
            
            # Compute OOS predictions (using your weighted average predictions)
            y_val_hat, _ = compute_oos_predictions(train_set, val_set, peer_weights, target_variable)
            m2b_pred = np.exp(y_val_hat)
            
            # Use the unscaled book_equity from the validation set to get predicted market cap
            book_equity_val = val_set['book_equity_unscaled']
            mktcap_pred = np.maximum(m2b_pred * book_equity_val.values, 0)
            
            # Compute RMSE between the predicted market cap and actual market cap
            val_rmse = np.sqrt(mean_squared_error(y_val_actual, mktcap_pred))
            return iteration, kmeans, k, val_rmse

        except Exception as e:
            print(f"Error encountered for {k} clusters: {e}")
            return iteration, None, k, np.inf

    # Run several iterations in parallel to search over different cluster counts
    results = Parallel(n_jobs=n_jobs)(delayed(evaluate_kmeans)(i) for i in range(iterations))
    results.sort(key=lambda x: x[0])

    # Report RMSEs over iterations
...
# Function to compute peer weights based on cluster assignment
def compute_peer_weights_kmeans(train_set, test_set, clustering_model, predictors):
    """
    Computes peer weights for each test firm based on clustering similarity.

    Parameters:
    train_set (pd.DataFrame): Training dataset containing firm data.
    test_set (pd.DataFrame): Test dataset containing firm data.
    clustering_model (KMeans): A trained K-Means clustering model.
    ...
    train_set['cluster'] = clustering_model.predict(train_set[predictors].to_numpy())
    test_set['cluster'] = clustering_model.predict(test_set[predictors].to_numpy())
...
for month in tqdm(unique_months, desc="Processing months", position=0, leave=True):
    ...
    for fold in tqdm(range(1, 6), desc="Processing folds", position=1, leave=True):
        ...
        # Optimize clusters for the current fold
        clustering_model, best_n_clusters, _ = optimize_clusters_kmeans(train_set, val_set, predictors)
        ...
        peer_weights = compute_peer_weights_kmeans(train_set, test_set, clustering_model, predictors)
        if peer_weights is None:
        ...
        # Save the W matrix
        W_path = f"/Users/hhsmid/Desktop/Hugo EUR/0. Master Thesis/05. Results/K-means/m2b/Peer weights/peer_weights_{month.strftime('%Y-%m')}_fold{fold}.csv"
        ...
        m2b_median_pred = np.exp(y_test_medians_hat)
        ...
        mktcap_median_pred = np.maximum(m2b_median_pred * book_equity_test.values, 0)
        ...
        fold_outcomes_path = f"/Users/hhsmid/Desktop/Hugo EUR/0. Master Thesis/05. Results/K-means/m2b/Outcomes/outcomes_{month.strftime('%Y-%m')}_fold{fold}.csv"
...
# Save the results
results.to_csv("/Users/hhsmid/Desktop/Hugo EUR/0. Master Thesis/05. Results/K-means/m2b/results.csv", index=False)
